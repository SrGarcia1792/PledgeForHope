...
/******************************************************************************************************************/
	public function select($strip)
		{
			$result = mysql_query("SELECT * FROM projects WHERE project_name LIKE '%$strip%'");	
			while($row = mysql_fetch_array($result))
			  {
				 echo "<img src=". $row['url_image'] . " width='208' height=116'><br/><br/>";
				 echo $row['project_name']."<br/><br/>";
				 echo $row['description']."<br/><hr/>";
			  }
		}
		
	public function select_select($strip)
		{	
			$result = mysql_query("SELECT * FROM projects WHERE categories = '$strip'");
			while($row = mysql_fetch_array($result))
			  {
				 echo "<img src=".$row['url_image']. " width='208' height=116' /><br/><br/>";
				 echo $row['project_name']." <br/><br/>";
...
/******************************************************************************************************************/
	...
	$conex = array("server" => "localhost" , "username" => "root", "password" => "", "dba" => "hope");
	$bd = new MySql($conex);
